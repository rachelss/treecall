...
python $pyfilter - 'AD:2;PL:60' | #pipe to filter to check both alleles have more than min reads AND both het and homo present AND likely more than one geno across all samples
bgzip -c > $dir/$dir.candidate1.vcf.gz  #pipe to zip; Skip lines started with character CHAR. [#]

#fix below to allow for gz
python $pyfilter $dir/$dir.candidate1.vcf.gz 'AD4:1' | bgzip -c > $dir/$dir.candidate2.vcf.gz #filter for at least F or R has 1 for both refs and alts
#not sure this is dif from filtering just AD

# infer tree
#fix below to allow for gz
#python $treecall part -m 60 -v 60 -e 30 $dir/$dir.candidate2.vcf.gz $dir/$dir.treecall-pt
python $treecall nbjoin -m 60 -v 60 -e 30 $dir/$dir.candidate2.vcf.gz $dir/$dir.treecall-nj

# treecall genotype
bcftools view $dir/$dir.bcf | python $pyfilter - 'AD:2' | python $treecall gtype -t $dir/$dir.treecall-pt.nwk -m 60 -e 30 - $dir/$dir.tc.txt
awk '$5>0.5' $dir/$dir.tc.txt > $dir/$dir.tc.p50.txt

bcftools view $dir/$dir.bcf | python $pyfilter - 'AD:2' | python $treecall gtype -t ms.nwk -m 60 -e 30 - $dir/$dir.ms.txt
awk '$5>0.5' $dir/$dir.ms.txt > $dir/$dir.ms.p50.txt
echo "treecall done"

# phylip
mkdir -p phylip
bcftools view -v snps $dir/$dir.snp.vcf.gz | python $mkphylip - phylip > phylip/$dir.snp.phylip
cd phylip

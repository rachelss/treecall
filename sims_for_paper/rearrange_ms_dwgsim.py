...
if __name__ == "__main__":
    ...
    pos = get_pos(msoutput[6].split()) #takes list of float-as-str, returns list of int
    
    handle = open('ref/chr22_20-21M.fa', "rU")
    ref = SeqIO.to_dict(SeqIO.parse(handle, "fasta"))
    handle.close()
    
    refbases = get_refbases(ref['chr22'].seq,pos)  #returns list of reference bases for each variant position in pos
